...
13a. A worktree listing reads porcelain once, then resolves every listed HEAD's commit time in one batched, read-only `git show`. Recent projects sort their worktrees by that time and sort each project by its newest worktree. Missing or stale HEADs remain visible without a timestamp.
