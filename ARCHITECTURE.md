...
- `src/git/github/commits.rs`: bounded pull-request commit pagination validated and cached against exact base and head OIDs.
...
- `src/watch.rs`: repository watcher, extra watch on the Git common directory so linked worktrees refresh, and event-storm coalescing.
- `src/state.rs`: recently opened project paths, keyed by Git common directory and ordered by the newest worktree commit.
...
1c. Recent-project enumeration and SSH reachability probes never block the first terminal render. The project picker renders skeleton rows until its background result arrives. Machine probes publish accessibility into the current workspace context without replacing its shared tab order, active tab, or pending new-project tab.
1d. File icons are static glyphs resolved by allocation-free, compile-time sorted hash catalogs in the render layer. Rendering never reads SVGs, font files, configuration, or the filesystem, and unknown paths use one generic glyph.
2. Every preview, status, history, branch, and pull-request request carries a generation; stale replies are ignored.
3. Fixed coalescing mailboxes isolate status/history, GitHub metadata (lookup, checks, check logs), paginated conversations, local change/commit/branch/stash previews, worktree and recent-project listings, and potentially network-bound PR previews. Background diff prefetch and stack-member cache warming use dedicated lanes, so neither can displace a read the user is waiting for. Key repeat remains constant-space, a large conversation cannot block interactive check logs, slow GitHub work cannot block tab switches, and ordered user mutations are serialized by app state. No GitHub command is queued at startup or merely by opening the PR tab.
4. Watcher signals are lossy by design: events settle for 250 milliseconds before one full status snapshot subsumes the burst, and scheduling the snapshot does not repaint the terminal.
5. Git/PR patch output is capped at 8 MiB per read. Exact PR/check metadata is capped at 2 MiB, pull-request commit lists at 500 commits across five newest-first pages, changed-file indexes at 8 MiB / 16,384 entries, conversations at 500 entries fetched newest-first in bounded pages (review comments descending, the timeline from its last Link page) so the cap can only ever drop the oldest activity, check logs at 8 MiB / 200,000 lines, repository discovery at 16 identities, and adaptive selected-PR history fetches at 16,384 commits. Background prefetch walks the whole index up to 4,096 files, starting at the file the Files tree is showing and wrapping around the rest in order, sizes each batch by per-file count estimates to stay under the 8 MiB patch read, and backfills a header's counts from its arrived patch when GitHub could not report them.
6. Potentially large local and PR subprocess output is read through capped pipes. Crossing a cap kills the child rather than first allocating all output and truncating afterward. Syntax grammar parsing stops at 512 KiB per patch or 32 KiB per row, parsed PR patches use a 32 MiB in-memory budget, and collapsed cached patches are not cloned into the combined document, so post-processing remains bounded after Git returns.
...
8. Working-tree groups, commits, branch comparisons, and stashes use one coalesced local workspace. A bounded name/status index produces collapsed headers first; the first path is prefetched silently, and later expansions run path-scoped Git commands whose documents remain cached for that workspace. Periodic status snapshots do not rebuild an unchanged comparison. Watcher storms preserve the rendered comparison, defer preview work until the latest coalesced status snapshot, and replace the document only when its refreshed patch is ready.
...
9. PR patches first use immutable base/head OIDs already present in the opened repository, which makes local-branch PR previews network-free. Missing objects fall back to fixed refs in one bare workspace under the cache/temp root: the GitHub PR-head ref is shallow/partial fetched, the merge base is resolved through the GitHub compare API and fetched as one depth-1 commit, a failed API resolution falls back to fetching the base ref and deepening both sides adaptively, per-file line counts come from the pull-request files endpoint instead of a blob-materializing local numstat, and the workspace remains alive while files are selected. Each selection runs a path-scoped Git diff; `Drop` removes the workspace. The opened repository receives no checkout, branch, ref, index, or worktree mutation.
10. The PR file tree is built once per index or directory-fold change from a bounded `git diff --name-status -z` index and renders only viewport rows. Its default all-files document reuses that index with collapsed headers and cached path documents; selecting a tree file enters single-file mode, while selecting the Files tab restores the index. Files and directories are selectable, directory rows expose real collapse/expand controls, and hidden descendants never trigger diff work. File-selection requests are generation-tagged and coalesced; there are no changed-file pages.
...
12b. Opening a pull request warms settled run logs in the background, on a dedicated lane behind every interactive read, capped at 32 stable GitHub job identities. The view carries persistent PR and conversation reload controls, changes their marks while reads are in flight, and says `cached` when what is on screen came from disk.
12c. Opening a pull-request stack warms every loaded member's check list and conversation into the bounded disk cache on a cancellable dedicated lane. Checks run before conversations, selected-member data is first, and commit lists and file patches remain demand-loaded.
...
13a. A worktree listing reads porcelain once, then resolves every listed HEAD's commit time in one batched, read-only `git log --no-walk`. Recent projects sort their worktrees by that time and sort each project by its newest worktree. Missing or stale HEADs remain visible without a timestamp.
...
15. The command line never repaints or streams progress into stdout or a pipe. One-shot commands may animate on interactive stderr, clearing the indicator before writing their answer or error. Human output is written once per invocation, `--json` is one document, and `--watch` is the single deliberate exception: one compact document per read under `--json`, a repainted screen on a terminal, and appended frames when redirected.
...
17. Completion scripts, manual pages, and capability discovery are metadata reads built directly from clap before repository discovery. Completions support bash, zsh, fish, elvish, and PowerShell. Manual generation fully builds one clap tree, then renders the root and nested commands from it so nested pages retain their full command path and inherited global options. Capability discovery serializes that same built tree for automation.
18. Self-update delegates a Homebrew-owned executable to `brew upgrade quinjet`. Other installations resolve one stable GitHub release before downloading anything else, pin its checksum and target binary to that tag, verify SHA-256 before staging beside the running executable, and replace the canonical executable so a `q` launcher symlink is not the destination. Unix replacement copies beside that path and renames; Windows uses `self-replace`. An older or equal release never replaces the binary, and any network, checksum, staging, or replacement failure leaves the running path intact.
