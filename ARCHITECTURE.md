...
18. Self-update resolves one stable GitHub release before downloading anything else, pins its checksum and target binary to that tag, verifies SHA-256 before staging beside the running executable, and replaces the canonical executable so a `q` launcher symlink is not the destination. Unix replacement copies beside that path and renames; Windows uses `self-replace`. An older or equal release never replaces the binary, and any network, checksum, staging, or replacement failure leaves the running path intact.
