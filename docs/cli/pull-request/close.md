...
```bash
quinjet pr close <number> [--repo <owner/name>] [--refresh] [--yes] [-C <DIR>] [--json]
```
...
```console
$ quinjet pr close 12
Would close #12 (Fix the thing). Pass --yes to close it.
```

With `--yes` it runs `gh pr close <number> --repo <url>` against the same
canonical repository URL every other `pr` verb uses. Closing an already closed
or merged pull request fails with whatever `gh` reports.
...
```bash
quinjet pr close 12
quinjet pr close 12 --yes
quinjet pr close 12 --repo pulkitxm/quinjet --yes --json
```

## Where to go next

- [`quinjet pr reopen`](./reopen.md) to undo a close
- [`quinjet pr merge`](./merge.md) to land the pull request instead
